...
import { type AiRouteMode, type AiSettings, type LlmProvider, type Settings } from '@/lib/types';
...
export function AiEngineSettings({ compact = false }: { compact?: boolean }) {
  ...
  const [selectionSettings, setSelectionSettings] = useState<Settings | null>(null);
  ...
  useEffect(() => {
    getSettings().then(setSelectionSettings);
    return watchSettings(setSelectionSettings);
  }, []);
  ...
  async function patchSelection(patchValue: Partial<Settings>) {
    setSelectionSettings(await setSettings(patchValue));
  }
  ...
  if (!ai || !selectionSettings) {
    return <div className="rounded-xl border border-line bg-surface-raised p-4 text-xs text-ink-faint">Loading AI settings…</div>;
  }
